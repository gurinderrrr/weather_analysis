...
combine_tday_03_mh=combine_tday_03_mh[['STATION','TEMP DAY MIN. (\'C)','TEMP. (\'C)','WIND DIR 10 m (Deg)','WIND SPEED 10 m (Kt)','RH (%)','MSLP (hPa / gpm)','GPS']]
                       ...
                       #replace names
combine_tday_03_mh.columns =combine_tday_03_mh.columns.str.replace('TEMP DAY MIN. (\'C)', 'MINT',regex=False)
...
combine_yday_mh=combine_yday_mh[['STATION','TEMP DAY MAX. (\'C)']]
                           ...
                           #replace names
combine_yday_mh.columns =combine_yday_mh.columns.str.replace('TEMP DAY MAX. (\'C)', 'MAXT',regex=False)
...
df['MINT'] = df['MINT'].astype(np.float64)
df['MAXT'] = df['MAXT'].astype(np.float64)
...
df = df[['S.No.', 'STATIONS','DISTRICT','TYPE', 'RF', 'MINT', 'MAXT', 'TEMP', 'RH (%)', 'WD', 'WS', 'MSLP', 'BAT', 'GPS']]
