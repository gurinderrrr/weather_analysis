...
try:
    ...
    today_mh = pd.read_html(f'http://aws.imd.gov.in:8091/AWS/dataview.php?a=AWSAGRO&b=MAHARASHTRA&c=ALL_DISTRICT&d=ALL_STATION&e={d1}&f={d1}&g=ALL_HOUR&h=ALL_MINUTE')[0]
    arg_today_mh = pd.read_html(f'http://aws.imd.gov.in:8091/AWS/dataview.php?a=ARG&b=MAHARASHTRA&c=ALL_DISTRICT&d=ALL_STATION&e={d1}&f={d1}&g=ALL_HOUR&h=ALL_MINUTE')[0]
...
start_datetime = d1 + ' 03:00:00'
end_datetime = d1 + ' 12:00:00'
frequency = '15min'
...
datetime_d1 = pd.to_datetime(d1 + ' 12:00:00')
...
#Collect today's data (adjust your URLs accordingly)
today_12_mh = pd.read_html(f'http://aws.imd.gov.in:8091/AWS/dataview.php?a=AWSAGRO&b=MAHARASHTRA&c=ALL_DISTRICT&d=ALL_STATION&e={d1}&f={d1}&g=12&h=00')[0]
arg_today_12_mh = pd.read_html(f'http://aws.imd.gov.in:8091/AWS/dataview.php?a=ARG&b=MAHARASHTRA&c=ALL_DISTRICT&d=ALL_STATION&e={d1}&f={d1}&g=12&h=00')[0]

#Combine the dataframes
combine_tday_12_mh = pd.concat([today_12_mh, arg_today_12_mh], ignore_index=True)

                            #drop vidarbha stations
mah_drop_12_today = combine_tday_12_mh[(combine_tday_12_mh['DISTRICT'] == 'AKOLA') | (combine_tday_12_mh['DISTRICT'] == 'AMRAVATI')|
(combine_tday_12_mh['DISTRICT'] == 'BHANDARA') | (combine_tday_12_mh['DISTRICT'] == 'BULDHANA')|
(combine_tday_12_mh['DISTRICT'] == 'CHANDRAPUR') | (combine_tday_12_mh['DISTRICT'] == 'GADCHIROLI')|
(combine_tday_12_mh['DISTRICT'] == 'GONDIA') | (combine_tday_12_mh['DISTRICT'] == 'NAGPUR')|
(combine_tday_12_mh['DISTRICT'] == 'YAVATMAL') | (combine_tday_12_mh['DISTRICT'] == 'WARDHA')|
(combine_tday_12_mh['DISTRICT'] == 'WASHIM')].index
combine_tday_12_mh.drop(mah_drop_12_today, inplace=True)


                        #choose columns to include in combine_tday_mh
combine_tday_12_mh=combine_tday_12_mh[['STATION','TEMP DAY MAX. (\'C)']]


                       #replace names
combine_tday_12_mh.columns =combine_tday_12_mh.columns.str.replace('TEMP DAY MAX. (\'C)', 'MAX T',regex=False)



combined_all_12_stations = all_stations_df.merge(combine_tday_12_mh, on='STATION', how='left')
...
#combine_rf_max=latest_data_per_station.merge(combined_all_12_stations, on='STATION', how='left')

combine_all=latest_data_per_station.merge(combined_all_12_stations, on='STATION', how='left')
...
#df.to_excel(os.path.join(os.path.join(os.environ['USERPROFILE']),'Downloads\\MAHARASHTRA ('+d0_2+' to '+d1_2+')''.xlsx'))
                        ...
                        #set index
dfy_index_mh=combine_all.set_index("STATION")
...
df_tot=len(aws_mh)
df_rep=len(aws_mh)-len(df[(df['AWS/ARG'] == 'AWS') & (df['null'] == 2)])
arg_df_tot=len(arg_mh)
arg_df_rep=len(arg_mh)-len(df[(df['AWS/ARG'] == 'ARG') & (df['null'] == 2)])



df_pg_hd_mh=pd.DataFrame(['STATE: MAHARASHTRA ('+d1_2+' 3UTC to '+d1_2+' 12UTC)'])
df_rf_leg_head_mh=pd.DataFrame({'':['LEGENDS']})
df_rf_leg_mh=pd.DataFrame({'':['Not Reported','1mm <= RF <= 2.4mm','2.5mm <= RF <= 15.5mm','15.6mm <= RF <= 64.4mm','64.5mm <= RF <= 115.5mm','115.6mm <= RF <= 204.4mm','RF > 204.4/\nHighest Temp','Error Values']})
df_rf_leg_col_mh=pd.DataFrame({'':['','','','','','','Highest Temp','Error Value']})
...
df = df[['S.No.','DISTRICT','STATIONS','AWS/ARG','RF','MAX T']]
...
try:
    #Collect today's data (adjust your URLs accordingly)
    today_goa = pd.read_html(f'http://aws.imd.gov.in:8091/AWS/dataview.php?a=AWSAGRO&b=GOA&c=ALL_DISTRICT&d=ALL_STATION&e={d1}&f={d1}&g=ALL_HOUR&h=ALL_MINUTE')[0]
    arg_today_goa = pd.read_html(f'http://aws.imd.gov.in:8091/AWS/dataview.php?a=ARG&b=GOA&c=ALL_DISTRICT&d=ALL_STATION&e={d1}&f={d1}&g=ALL_HOUR&h=ALL_MINUTE')[0]
...
combine_tday_goa = pd.concat([today_goa, arg_today_goa], ignore_index=True)
                            ...
                            #drop vidarbha stations
goa_drop_today = combine_tday_goa[(combine_tday_goa['DISTRICT'] == 'AKOLA') | (combine_tday_goa['DISTRICT'] == 'AMRAVATI')|
(combine_tday_goa['DISTRICT'] == 'BHANDARA') | (combine_tday_goa['DISTRICT'] == 'BULDHANA')|
(combine_tday_goa['DISTRICT'] == 'CHANDRAPUR') | (combine_tday_goa['DISTRICT'] == 'GADCHIROLI')|
(combine_tday_goa['DISTRICT'] == 'GONDIA') | (combine_tday_goa['DISTRICT'] == 'NAGPUR')|
(combine_tday_goa['DISTRICT'] == 'YAVATMAL') | (combine_tday_goa['DISTRICT'] == 'WARDHA')|
(combine_tday_goa['DISTRICT'] == 'WASHIM')].index
combine_tday_goa.drop(goa_drop_today, inplace=True)
...
#Collect today's data (adjust your URLs accordingly)
today_12_goa = pd.read_html(f'http://aws.imd.gov.in:8091/AWS/dataview.php?a=AWSAGRO&b=GOA&c=ALL_DISTRICT&d=ALL_STATION&e={d1}&f={d1}&g=12&h=00')[0]
arg_today_12_goa = pd.read_html(f'http://aws.imd.gov.in:8091/AWS/dataview.php?a=ARG&b=GOA&c=ALL_DISTRICT&d=ALL_STATION&e={d1}&f={d1}&g=12&h=00')[0]

#Combine the dataframes
combine_tday_12_goa = pd.concat([today_12_goa, arg_today_12_goa], ignore_index=True)

                            #drop vidarbha stations
mah_drop_12_today = combine_tday_12_goa[(combine_tday_12_goa['DISTRICT'] == 'AKOLA') | (combine_tday_12_goa['DISTRICT'] == 'AMRAVATI')|
(combine_tday_12_goa['DISTRICT'] == 'BHANDARA') | (combine_tday_12_goa['DISTRICT'] == 'BULDHANA')|
(combine_tday_12_goa['DISTRICT'] == 'CHANDRAPUR') | (combine_tday_12_goa['DISTRICT'] == 'GADCHIROLI')|
(combine_tday_12_goa['DISTRICT'] == 'GONDIA') | (combine_tday_12_goa['DISTRICT'] == 'NAGPUR')|
(combine_tday_12_goa['DISTRICT'] == 'YAVATMAL') | (combine_tday_12_goa['DISTRICT'] == 'WARDHA')|
(combine_tday_12_goa['DISTRICT'] == 'WASHIM')].index
combine_tday_12_goa.drop(mah_drop_12_today, inplace=True)


                        #choose columns to include in combine_tday_goa
combine_tday_12_goa=combine_tday_12_goa[['STATION','TEMP DAY MAX. (\'C)']]


                       #replace names
combine_tday_12_goa.columns =combine_tday_12_goa.columns.str.replace('TEMP DAY MAX. (\'C)', 'MAX T',regex=False)



combined_all_12_stations = all_stations_df.merge(combine_tday_12_goa, on='STATION', how='left')
...
#df.to_excel(os.path.join(os.path.join(os.environ['USERPROFILE']),'Downloads\\GOA ('+d0_2+' to '+d1_2+')''.xlsx'))
                        ...
                        #set index
dfy_index_goa=combine_all.set_index("STATION")
...
df_tot=len(aws_goa)
df_rep=len(aws_goa)-len(df[(df['AWS/ARG'] == 'AWS') & (df['null'] == 2)])
arg_df_tot=len(arg_goa)
arg_df_rep=len(arg_goa)-len(df[(df['AWS/ARG'] == 'ARG') & (df['null'] == 2)])



df_pg_hd_goa=pd.DataFrame(['STATE: GOA ('+d1_2+' 3UTC to '+d1_2+' 12UTC)'])
...
def highlight_single_cells(x):
    ...
    df_rf_leg_col.iloc[8,0] = 'background-color: #FF0000;color: yellow;font-weight:bold'
    return df_rf_leg_col
...
try:
    #Collect today's data (adjust your URLs accordingly)
    today_gj = pd.read_html(f'http://aws.imd.gov.in:8091/AWS/dataview.php?a=AWSAGRO&b=GUJARAT&c=ALL_DISTRICT&d=ALL_STATION&e={d1}&f={d1}&g=ALL_HOUR&h=ALL_MINUTE')[0]
    arg_today_gj = pd.read_html(f'http://aws.imd.gov.in:8091/AWS/dataview.php?a=ARG&b=GUJARAT&c=ALL_DISTRICT&d=ALL_STATION&e={d1}&f={d1}&g=ALL_HOUR&h=ALL_MINUTE')[0]
...
combine_tday_gj = pd.concat([today_gj, arg_today_gj], ignore_index=True)
                            ...
                            #drop vidarbha stations
gj_drop_today = combine_tday_gj[(combine_tday_gj['DISTRICT'] == 'AKOLA') | (combine_tday_gj['DISTRICT'] == 'AMRAVATI')|
(combine_tday_gj['DISTRICT'] == 'BHANDARA') | (combine_tday_gj['DISTRICT'] == 'BULDHANA')|
(combine_tday_gj['DISTRICT'] == 'CHANDRAPUR') | (combine_tday_gj['DISTRICT'] == 'GADCHIROLI')|
(combine_tday_gj['DISTRICT'] == 'GONDIA') | (combine_tday_gj['DISTRICT'] == 'NAGPUR')|
(combine_tday_gj['DISTRICT'] == 'YAVATMAL') | (combine_tday_gj['DISTRICT'] == 'WARDHA')|
(combine_tday_gj['DISTRICT'] == 'WASHIM')].index
combine_tday_gj.drop(gj_drop_today, inplace=True)
...
#Collect today's data (adjust your URLs accordingly)
today_12_gj = pd.read_html(f'http://aws.imd.gov.in:8091/AWS/dataview.php?a=AWSAGRO&b=GUJARAT&c=ALL_DISTRICT&d=ALL_STATION&e={d1}&f={d1}&g=12&h=00')[0]
arg_today_12_gj = pd.read_html(f'http://aws.imd.gov.in:8091/AWS/dataview.php?a=ARG&b=GUJARAT&c=ALL_DISTRICT&d=ALL_STATION&e={d1}&f={d1}&g=12&h=00')[0]

#Combine the dataframes
combine_tday_12_gj = pd.concat([today_12_gj, arg_today_12_gj], ignore_index=True)

                            #drop vidarbha stations
mah_drop_12_today = combine_tday_12_gj[(combine_tday_12_gj['DISTRICT'] == 'AKOLA') | (combine_tday_12_gj['DISTRICT'] == 'AMRAVATI')|
(combine_tday_12_gj['DISTRICT'] == 'BHANDARA') | (combine_tday_12_gj['DISTRICT'] == 'BULDHANA')|
(combine_tday_12_gj['DISTRICT'] == 'CHANDRAPUR') | (combine_tday_12_gj['DISTRICT'] == 'GADCHIROLI')|
(combine_tday_12_gj['DISTRICT'] == 'GONDIA') | (combine_tday_12_gj['DISTRICT'] == 'NAGPUR')|
(combine_tday_12_gj['DISTRICT'] == 'YAVATMAL') | (combine_tday_12_gj['DISTRICT'] == 'WARDHA')|
(combine_tday_12_gj['DISTRICT'] == 'WASHIM')].index
combine_tday_12_gj.drop(mah_drop_12_today, inplace=True)


                        #choose columns to include in combine_tday_gj
combine_tday_12_gj=combine_tday_12_gj[['STATION','TEMP DAY MAX. (\'C)']]


                       #replace names
combine_tday_12_gj.columns =combine_tday_12_gj.columns.str.replace('TEMP DAY MAX. (\'C)', 'MAX T',regex=False)



combined_all_12_stations = all_stations_df.merge(combine_tday_12_gj, on='STATION', how='left')
...
#df.to_excel(os.path.join(os.path.join(os.environ['USERPROFILE']),'Downloads\\gj ('+d0_2+' to '+d1_2+')''.xlsx'))
                        ...
                        #set index
dfy_index_gj=combine_all.set_index("STATION")
...
df_tot=len(aws_gj)
df_rep=len(aws_gj)-len(df[(df['AWS/ARG'] == 'AWS') & (df['null'] == 2)])
arg_df_tot=len(arg_gj)
arg_df_rep=len(arg_gj)-len(df[(df['AWS/ARG'] == 'ARG') & (df['null'] == 2)])



df_pg_hd_gj=pd.DataFrame(['STATE: GUJARAT ('+d1_2+' 3UTC to '+d1_2+' 12UTC)'])
