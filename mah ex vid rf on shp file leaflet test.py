...
# Save to GeoJSON
geojson_path = 'shapefile.geojson'
...
# Create a base map
m = folium.Map(location=[19.0, 73.0], zoom_start=7, tiles=None)

# Add GeoJSON layer with black borders
geojson_path = 'shapefile.geojson'
folium.GeoJson(
    geojson_path,
...
def get_color(rainfall):
    if pd.isna(rainfall):
        ...
        return '#98FB98'
    elif 2.6 <= rainfall <= 15.5:
        return '#7FFF00'
    elif 15.6 <= rainfall <= 64.5:
        return '#228B22'
    elif 64.6 <= rainfall <= 115.5:
        return '#FFFF00'
    elif 115.6 <= rainfall <= 204.5:
        return '#FF8C00'
    else:
        return '#FF0000'
...
for _, row in df_geo.iterrows():
    folium.CircleMarker(
        ...
        radius=1,
        color=get_color(row['RF']),
        fill=True,
        fill_color=get_color(row['RF']),
        fill_opacity=1,
        popup=folium.Popup(f"{row['STATIONS']}: {row['RF']} mm", parse_html=True)
    ).add_to(m)


# Save the map to an HTML file
m.save('C:\\Users\\hp\\Desktop\\gurinder\\python test\\plotly awarg test.html')
