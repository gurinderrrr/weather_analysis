...
#yesterday = (datetime.datetime.utcnow() - datetime.timedelta(days=1)).strftime("%d %b, %Y")
...
<head>
    <style>
        h2 {
            ...
            th, td {
            min-width: 50px;  /* Set a minimum width for table cells to prevent shrinking */
        }
        td {
            white-space: nowrap;  /* Prevent shrinking of empty columns */
        }
        table {
            margin: 0 auto;
...
# Iterate over each district to create district-wise tables without column headers
for i ,district in enumerate(df['DISTRICT'].unique()):

    # Add district header
    html_output += f'<h2>{district} ({d0_2} 03UTC to {d1_2} 03UTC)</h2>\n'
    ...
    # Apply styles to the filtered district DataFrame (without column headers)
    styled_district_df = district_df.style \
        ...
        .hide(axis='index') \
        .hide(axis='columns')  # Hide column headers

    # Convert the styled district DataFrame to HTML
    district_html = styled_district_df.format(precision=1, na_rep="").to_html(escape=False)
    
    # Inject the modified HTML into the final output
...
combined_html_file = 'combined_table_without_headers.html'
...
options = {
    ...
    'margin-right': '5mm',
    'page-size': 'A4',
    'footer-center': '[page]',  # Display the page number on the right side in the footer
    'footer-font-size': '8',  # Set footer font size
}
